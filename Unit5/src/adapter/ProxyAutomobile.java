...
public abstract class ProxyAutomobile extends Thread {
	/*
	...
	public void buildAuto(Object fileName, String fileType) throws Exception {
		...
		try {
			...
			System.out.println("read " + newModel.getModel() + " success");
	...
	public void buildCarModelOptions() throws Exception {
		// handle the client request from server;
		...
		while (true) {
			...
			if (receivedObject instanceof Properties) {
				// Upload propertes and save to linkedhashmap
				...
				System.out.println("build automotive successfully from server");
				in.close();
				out.close();
			} else {
				...
				} else if (content.equals("getModel")) {
					// return selected auto
					String autoName = msg.getAutoName();
					Automobile auto = getAutoMobile(autoName);
					out.writeObject(auto);
					System.out.println(
							"get client model selection info successfully");
				} else if (content.equals("setOption")) {
					// return options
					String autoName = msg.getAutoName();
					Automobile auto = getAutoMobile(autoName);
					ArrayList<String> options = msg.getOptions();
					for (int i = 0; i < options.size(); i++) {
						auto.setOptionChoice(auto.getOptionSetName(i),
								options.get(i));
					}
					out.writeObject(auto);
					System.out.println("set selection info successfully");
				}
